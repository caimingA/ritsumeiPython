...
def compute_cos_before(documents, inv, keys):
...
def compute_cos_after(documents, inv, keys):
    doc_num = len(documents)
    cos_matrix = [[0 for i in range(doc_num)] for j in range(doc_num)]
    for i in range(doc_num):
        cos_matrix[i][i] = 1.0
        for j in range(i + 1, doc_num):
            ...
            # print(IDa)
            for key in keys:
                if IDa in inv[key] and IDb in inv[key]:
                    dotProduct += int(inv[key][IDa]) * int(inv[key][IDb])
            cos = dotProduct / (documents[i].get_length() * documents[j].get_length())
            cos_matrix[i][j] = cos_matrix[j][i] = cos
    return cos_matrix
...
if __name__ == '__main__':
    ...
    starttime = time.time()
    cos = compute_cos_before(documents, inv, keys)
    endtime = time.time()
    print("改善前のCOS計算時間：", (endtime - starttime), 's')

    starttime = time.time()
    cos = compute_cos_after(documents, inv, keys)
    endtime = time.time()
    print("改善後のCOS計算時間：", (endtime - starttime), 's')
    # print(cos)
